...
const porcentaje = (cantidad) => {
  ...
  const total = chartStore.totalMuestras;
...
<template>
  <div class="contenedor">
    <h1 class="titulo__graficos">
      Muestras tomadas:
      <span class="grafico__cantidad">{{ chartStore.separarMiles(chartStore.totalMuestras) }}</span>
    </h1>
    <div class="graficos">
      <div
      ...
      >
        <div
          :class="{
            grafico: true,
            menor: porcentaje(chartStore.totalDepartamentos[index]) < 50,
          }"
          :style="{
            '--i': porcentaje(chartStore.totalDepartamentos[index]),
            '--clr': colores[index],
          }"
        >
          <h3 class="grafico__porcentaje">
            {{ porcentaje(chartStore.totalDepartamentos[index]) }} %
          </h3>
          <h4 class="grafico__departamento">
            {{ departamento }}
            <span class="grafico__cantidad" id="grafico__cantidad">
              {{
                chartStore.separarMiles(chartStore.totalDepartamentos[index])
              }}

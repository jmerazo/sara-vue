...
import ChartCalendar from "@/components/species/calendars/ChartCalendar.vue";
const calendar = useCalendarStore();

onMounted(() => {
  const content = document.querySelector("#flowerContent");
  const buttonLeft = document.querySelector("#leftFlower");
  const buttonRight = document.querySelector("#rightFlower");
...
<template>
  <div v-if="calendar.dataFlower.length !== 0">
    <h3 style="text-align: center; color: var(--gris); margin-top: 5rem">
      Calendario Floral
    </h3>
    <div class="slider__calendar">
      <div class="slider__content" ref="sliderContent" id="flowerContent">
        <div
          class="card__content"
          v-for="(item, index) in calendar.dataFlower"
          :key="index"
        >
          <div class="slider__card">
            ...
            <div class="card__values">
              <div class="values">
                <ChartCalendar :item="item" :index="index" :name="'flower'" />
              </div>
            </div>
          </div>
        </div>
        <div class="slider__buttons">
          <button class="slider__button slider__button--left" id="leftFlower" ref="buttonLeft">
            <svg
              style="width: 22px"
